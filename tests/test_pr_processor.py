...
def test_process_pull_request_flow():
    with patch("prfectbot.pr_processor.clone_pr_branch") as mock_clone, patch(
        "prfectbot.pr_processor.run_linter_formatter"
    ) as mock_lint, patch(
        "prfectbot.pr_processor.commit_and_push_fixes"
    ) as mock_commit, patch(
        "prfectbot.pr_processor.post_pr_comment"
    ) as mock_comment:
        mock_clone.return_value = True
        mock_lint.return_value = True
        mock_commit.return_value = True

        assert process_pull_request("octocat", "Hello-World", "feature", 101) is True
