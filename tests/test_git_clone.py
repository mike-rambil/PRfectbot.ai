...
def test_clone_pr_branch_success():
    with patch("prfectbot.gitutils.run_git_clone") as mock_clone:
        ...
        result = clone_pr_branch(
            "octocat", "Hello-World", "feature-branch", "/tmp/clone"
        )
        mock_clone.assert_called_once_with(
            "https://github.com/octocat/Hello-World.git", "feature-branch", "/tmp/clone"
        )
...
def test_run_git_clone_success():
    with patch("subprocess.run") as mock_run:
        ...
        assert (
            run_git_clone(
                "https://github.com/octocat/Hello-World.git", "main", "/tmp/clone"
            )
            is True
        )
